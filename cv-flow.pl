...
foreach (@imgs) {
    ...
    unless (($n+$k)%5) {
        ...
        if ($method eq 'flow') {
            ...
            print `$xf $_ $next $fflow`;
            print "flow from $next to $_\n";
            print `$xf $next $_ $bflow`;
            print "twoway interpolation\n";
            print `$si twoway $_ $next $fflow $bflow .out%1$glob 0 2`;
            print "saving center interpolation\n";
            print `mv .out00000001$glob $nexf`;
            print `rm -rf .out*`;
            ...
            print `$si forward $_ $fflow .out%1$glob 0 2`;
            print "saving center interpolation\n";
            print `mv .out00000001$glob $nexf`;
            print `rm -rf .out*`;
            ...
            print `convert $_ $next -average -flatten $nexf`;
